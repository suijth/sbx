...
class RecurrentPPO(OnPolicyAlgorithmJax):
    ...
    def _setup_model(self) -> None:
        ...
        self._last_lstm_states = RNNStatesNp(
            (
                np.zeros(single_hidden_state_shape, device=self.device),
                np.zeros(single_hidden_state_shape, device=self.device),
            ),
            (
                np.zeros(single_hidden_state_shape, device=self.device),
...
if __name__ == "__main__":
    ...
    x = jnp.ones((5, 4))
    layer = MultiLayerLSTMCell(features=256, 
                kernel_init=ini,
                bias_init=ini,
                recurrent_kernel_init=ini)
    net = nn.RNN(layer, return_carry=True)
    vars1 = net.init(key, x)
    out1, c1 = net.apply(vars1, x)
